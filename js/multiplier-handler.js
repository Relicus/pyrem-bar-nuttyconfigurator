...
function getMultiplierCommands(multipliersConfig, getMultiplierValues) {
    ...
    multipliersConfig.forEach(section => {
        section.multipliers.forEach(m => {
            if (window.CommandBuilderSelectionScanner
                && window.CommandBuilderSelectionScanner.shouldEmitMultiplierCommand
                && !window.CommandBuilderSelectionScanner.shouldEmitMultiplierCommand(m)) {
                return;
            }
...
function generateLuaTweak(type, multiplierValue) {
    ...
    try {
        ...
        if (!payloadCodec || typeof payloadCodec.minifyLua !== 'function' || typeof payloadCodec.encodePayload !== 'function') {
            throw new Error('PayloadCodec not loaded');
        }

        const minifiedCode = payloadCodec.minifyLua(originalCode, {
            logStats: false,
            logErrors: false
        });
        const finalCodeToEncode = firstLineComment + '\n' + minifiedCode;
        return payloadCodec.encodePayload(finalCodeToEncode, {
            minify: false,
            logErrors: true
        });
    } catch (e) {
...
window.getMultiplierCommandsImpl = getMultiplierCommandsImpl;
window.generateLuaTweakImpl = generateLuaTweakImpl;
